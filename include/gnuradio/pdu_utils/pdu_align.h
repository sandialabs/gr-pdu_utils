...
{
public:
    typedef std::shared_ptr<pdu_align> sptr;
     ...
     * @param match_mode - mode to operate when sync is found
     *  ALIGN_FIRST_MATCH - align to first match found
     *  ALIGN_BEST_MATCH - read up to half the syncword length ahead for
     *                     a better match, recurses until limit is reached
     *                     or perfect match is found
     */
    static sptr
    make(std::string syncwords, int threshold, int offset, align_modes mode = ALIGN_DROP, align_match_mode match_mode = ALIGN_FIRST_MATCH);
};
