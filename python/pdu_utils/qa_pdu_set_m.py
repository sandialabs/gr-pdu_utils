...
class qa_pdu_set_m (gr_unittest.TestCase):
    ...
    def setUp (self):
        ...
        self.set = pdu_utils.pdu_set_m(pmt.intern('num'), pmt.from_long(4), False, True)
    ...
    def test_003_parse_val (self):
        in_data = [0, 0, 0, 0]
        in_meta = pmt.dict_add(pmt.make_dict(), pmt.intern('num'), pmt.from_long(4))
        expected_meta = pmt.dict_add(in_meta, pmt.intern('num_is'), pmt.intern('num_is_44_test'))
        in_pdu = pmt.cons(in_meta, pmt.init_u8vector(len(in_data), in_data))
        expected_pdu = pmt.cons(expected_meta, pmt.init_u8vector(len(in_data), in_data))
        ...
        self.emitter.emit(pmt.intern("MALFORMED PDU"))
        time.sleep(.001)
        self.set.set_kv(pmt.intern('num_is'), pmt.intern('num_is_{num}{num}_{test}'))
        self.set.set_kv_merge(True)
        self.emitter.emit(in_pdu)
        time.sleep(.01)
        self.tb.stop()
        self.tb.wait()

        print("out pdu =  ",self.debug.get_message(0))
